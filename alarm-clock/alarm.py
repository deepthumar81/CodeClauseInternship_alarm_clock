...
class Alarm(tk.Frame):
    ...
    def add_widgets(self):
        ...
        self.message_frame = tk.LabelFrame(self.label_frame, text="Alarm Message:")
        self.message_box = tk.Text(
            self.message_frame, wrap="word", height=3, width=15, relief=tk.RAISED
        )
        self.active_frame = tk.Frame(self)
        self.alarm_repeat = tk.Listbox(
            self.active_frame,
            height=0,
        ...
        self.active_button = tk.Checkbutton(
            self.active_frame,
            text="Active?",
        ...
        self.edit_button = tk.Button(
            self.edit_frame, text="Edit", command=self.edit_alarm
        )
        self.delete_button = tk.Button(
            self.edit_frame, text="Delete", command=self.delete
        )
        self.test_button = tk.Button(
            self.edit_frame, text="Test", command=self.play_sound
        )
    ...
    def edit_alarm(self):
        ...
        self.minutes_frame = tk.LabelFrame(self.spinbox_frame, text="Set minute:")
        self.minutes_select = tk.Spinbox(
            self.minutes_frame,
            from_=0,
            to=59,
        ...
        self.set_am = tk.Radiobutton(
            self.am_pm_frame, text="AM", variable=self.am_pm, value=0
        )
        self.set_pm = tk.Radiobutton(
            self.am_pm_frame, text="PM", variable=self.am_pm, value=1
        )
        self.active_button = tk.Checkbutton(
            self.edit_frame, text="Set Active:", variable=self.new_active
        )
        self.save_button = tk.Button(
            self.edit_window, text="Save", command=self.confirm_edit
        )
        self.cancel_button = tk.Button(
            self.edit_window, text="Cancel", command=self.edit_window.destroy
        )
